...
def buildSecondOfMinute(s, chrono_id, chrono_list, flags):
    ...
    if b and not flags["second"]:
        ...
        chrono_entity = chrono.ChronoSecondOfMinuteEntity(entityID=str(chrono_id) + "entity", start_span=abs_StartSpan,
        ...
        for text in text_list:
            ...
            if (re.search('^(?:(?:([01]?\d|2[0-3]):)?([0-5]?\d):)?([0-5]?\d)$', text)):  # checks for HH:MM:SS String
                ...
                if len(match.split(":")) == 3:
                    start_idx, end_idx = Chrono.ChronoUtils.parse_text.calculateSpan(text_norm, re.compile(":").split(match)[2])
